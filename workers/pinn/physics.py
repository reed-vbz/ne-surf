...
Differentiable wave physics shared by the loss function, the teacher and the inference fallback (PyTorch).
  ...
  friction     dE/dt_group = −C_f · E / h                   (empirical damping velocity C_f in m/s, unvalidated)
...
def wavenumber(h: torch.Tensor, T: torch.Tensor, iters: int = 16) -> torch.Tensor:
    """k from ω² = g k tanh(k h) by Newton iteration (vectorised, differentiable)."""
    h = h.clamp_min(0.05); w = 2 * math.pi / T
    k = torch.maximum(w * w / G, w / torch.sqrt(G * h))
...
def energy_balance_residual(H: torch.Tensor, theta: torch.Tensor, h: torch.Tensor, T: torch.Tensor, water: torch.Tensor, dx: float) -> torch.Tensor:
    ...
    D_friction = C_f E / h, D_breaking = c_g E · max(0, 1 − (γ h / H)²) / dx (energy above the breaking limit is lost).
    Gradients use central differences over water cells only (land neighbours are excluded).
    """
    dy, dx = (dx, dx) if isinstance(dx, (float, int)) else dx
    E = (H * H) / 16
    cg = group_speed(h, T)
    fx = cg * E * torch.sin(theta) * water; fy = cg * E * torch.cos(theta) * water
    dfx = torch.zeros_like(fx); dfy = torch.zeros_like(fy)
    dfx[..., 1:-1] = (fx[..., 2:] - fx[..., :-2]) / (2 * dx)
    dfy[..., 1:-1, :] = (fy[..., 2:, :] - fy[..., :-2, :]) / (2 * dy)
    interior = torch.zeros_like(water)
    interior[...,1:-1,1:-1] = water[...,1:-1,1:-1] * water[...,:-2,1:-1] * water[...,2:,1:-1] * water[...,1:-1,:-2] * water[...,1:-1,2:]
    div = dfx + dfy
    d_fric = CF * E / h.clamp_min(1.0) * (h < 1.56 * T * T * 1.5)
    over = (1 - (GAMMA * h.clamp_min(0.05) / H.clamp_min(1e-3)) ** 2).clamp_min(0)
    d_break = cg * E * over / dx
    scale = (cg * E / dx).clamp_min(1e-6)
    return ((div + d_fric + d_break) / scale) * interior
