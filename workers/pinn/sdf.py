...
STRIDE = 3   # 3 × 3″ CRM cells ≈ 270 m: the same grid the wave-field textures use
...
def load_depth(stride: int = STRIDE):
    ...
    res_m = res_deg * 111320 * math.cos(math.radians(j["lat0"] + a.shape[0] * res_deg / 2))
    return a, float(j["lat0"]), float(j["lon0"]), float(res_deg), float(res_m)
...
def signed_distance(water: np.ndarray, res_m: float) -> np.ndarray:
    ...
    return np.where(water, ndimage.distance_transform_edt(water), -ndimage.distance_transform_edt(~water)).astype(np.float32) * res_m
...
def build(stride: int = STRIDE):
    ...
    np.savez_compressed(OUT, depth=depth, sdf=sdf, water=water, lat0=lat0, lon0=lon0, res_deg=res_deg, res_m=res_m)
    return depth, sdf, water, res_m


if __name__ == "__main__":
    d, s, w, r = build()
    print(f"geometry {d.shape} at {r:.0f} m, water {w.mean():.2f}, sdf {s.min():.0f}…{s.max():.0f} m → {OUT}")
