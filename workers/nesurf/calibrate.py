"""
Compare GFS-Wave at each NDBC buoy with what the buoy actually measured, and write a per-buoy
bias correction the score engine applies to nearby spots.
...
bias and a direction bias. With fewer than `--min-pairs` pairs the ratio is reported but flagged low_confidence.

This corrects the *deep-water model* against a *deep-water buoy*. It says nothing about shoaling —
that is the bathymetry term (nesurf.bathy). Keep the two separate.
"""
...
def interp_model(series: list[dict], when: datetime, key: str) -> float | None:
    pts = [(_t(r["valid_time"]), r.get(key)) for r in series if r.get(key) is not None]
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--min-pairs", type=int, default=24, help="pairs before a ratio is trusted (one cycle gives ~13 autocorrelated pairs)")
    ...
    for b in ww3.get("buoys", []):
        ...
        if obs.get("obs_time") and obs.get("wvht_m") is not None:
            rows.append({"t": obs["obs_time"], "wvht_m": obs["wvht_m"], "dpd_s": obs.get("dpd_s"), "mwd_deg": obs.get("mwd_deg")})
        seen = set()
        for r in rows:
    ...
    # merge, dedupe on (buoy, t, cycle), drop old
    key = lambda p: (p["buoy"], p["t"], p["cycle"])
    ...
    pairs = [p for p in merged.values() if _t(p["t"]) >= cutoff]
    ...
    for bid in sorted({p["buoy"] for p in pairs}):
        ...
        buoys[bid] = {
            "n_pairs": len(ps), "low_confidence": len(ps) < a.min_pairs,
        ...
        log.info("%s n=%d obs %.2f m model %.2f m ratio %.2f (%s)", bid, len(ps), oh.mean(), mh.mean(), ratio, "low confidence" if len(ps) < a.min_pairs else "ok")
