...
| L1 wave energy | **Groundswell: the Rolling Wavefronts shader** (`components/map/RollingWavefrontsLayer.ts`, a BitmapLayer subclass with a custom fragment shader) over the backend wave field: `workers/nesurf/wavefield.py` solves the eikonal equation \|∇t\| = 1/c(h, T) by fast marching on the CRM grid (270 m) from the deep-water WW3 boundary, with c from the dispersion relation, then H_s by linear shoaling, bottom friction and γ-breaking; one RGBA texture per WW3 step (R,G = travel time 16-bit, B = H_s) published with the cache. The shader draws crests as level sets of φ = ω·t(x) − ω·u_time: continuous, spaced by the local wavelength, slowing and bending around headlands, aligning to coves, fading to zero at the shoreline, and it runs under the land layer. `workers/pinn` is the SWAN-PINN surrogate (U-Net/ConvLSTM, physics loss, FastAPI) that can replace the solver via `NEXT_PUBLIC_PINN_API`. **Wind waves**: the WW3 wind-sea partition as faint fast white particle dashes. | deck.gl custom layer + `TripsLayer` |
