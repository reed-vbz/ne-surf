...
interface Props {
  ...
  activeTime: number | null; onPickTime: (h: number | null) => void; days: DayRow[]; onPickDay: (i: number) => void;
...
const label: React.CSSProperties = { fontSize: 11, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: "#9fb1bc" };
...
export default function ForecastDrawer({ open, onClose, spotName, tier, dayLabel, hours, tide, activeTime, onPickTime, days, onPickDay, windAt, defaultTime }: Props) {
  const [tab, setTab] = useState<"hourly" | "daily">("hourly");
  const onMove = (e: PointerEvent<HTMLDivElement>) => { const r = e.currentTarget.getBoundingClientRect(); const f = (e.clientX - r.left - (PADL / W) * r.width) / (((X1 - X0) / W) * r.width); onPickTime(hours[Math.max(0, Math.min(hours.length - 1, Math.round(f * (hours.length - 1))))]?.t ?? null); };
  const shown = activeTime ?? defaultTime;
  const cur = shown !== null ? hours.find((p) => p.t === shown) ?? null : hours[Math.min(hours.length - 1, 12)] ?? null;
  ...
  const cursor = (h: number) => activeTime !== null ? <line x1={xT(activeTime)} x2={xT(activeTime)} y1={0} y2={h} stroke="#ffffff" strokeOpacity={0.5} strokeDasharray="2 3" /> : null;
  ...
  return (
    <>
      <style>{`.nesurf-drawer{position:absolute;left:8px;right:8px;bottom:8px;top:38%;z-index:8;display:flex;flex-direction:column;overflow:hidden;transform:translateY(${open ? 0 : "calc(100% + 16px)"});transition:transform 260ms cubic-bezier(.2,.8,.2,1)}
        @media (min-width:900px){.nesurf-drawer{left:auto;top:118px;right:12px;bottom:216px;width:380px;transform:translateX(${open ? 0 : "calc(100% + 24px)"})}}
        .nesurf-drawer-body{overflow-y:auto;scrollbar-width:thin;flex:1;min-height:0}`}</style>
      <section aria-label="Surf forecast" aria-hidden={!open} inert={!open} className="nesurf-drawer backdrop-blur-xl bg-slate-900/95 border border-white/10 rounded-2xl p-4 text-white shadow-2xl"
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#9fb1bc", letterSpacing: ".06em", textTransform: "uppercase" }}>{dayLabel}{cur ? ` · ${tick(cur.hour)}` : ""}</div>
          </div>
        ...
        {tab === "hourly" ? (
          <div className="nesurf-drawer-body" onPointerMove={(e) => { if (e.buttons) onMove(e); }} onPointerDown={onMove}  style={{ display: "flex", flexDirection: "column", gap: 10, touchAction: "pan-y" }}>
            {hours.length === 0 ? <div style={{ fontSize: 11, color: "#9fb1bc" }}>No hourly data for this day yet.</div> : <>
              <label style={{ fontSize: 12, color: "#cfe0ea" }}>Forecast time
                <input aria-label="Forecast time" type="range" min={0} max={Math.max(0, hours.length - 1)} step={1}
                  value={Math.max(0, hours.findIndex((p) => p.t === shown))} onPointerDown={(e) => e.stopPropagation()} onPointerMove={(e) => e.stopPropagation()}
                  onChange={(e) => onPickTime(hours[Number(e.target.value)]?.t ?? null)} style={{ display: "block", width: "100%", height: 32 }} />
              </label>
              <div style={{ fontSize: 12 }}>Quality {cur ? `${Math.round(cur.score)}/100` : "unavailable"} · estimated, not locally validated</div>
              {cur && cur.faceFt >= 0.5 && <div style={{ borderRadius: 12, background: "linear-gradient(180deg, rgba(0,229,255,.06), rgba(11,25,44,.35))", border: "1px solid rgba(255,255,255,.08)", padding: "4px 6px 2px" }}><WaveAvatar faceFt={cur.faceFt} tpS={cur.tp} wind={windAt(cur)} width={320} /></div>}
              <Row title="Estimated face height" value={cur ? faceLabel(cur.faceFt) : "—"}>
                <path d={`${path(hours.map((p) => [xT(p.t), 54 - (p.faceFt / maxFace) * 50]))} L${xT(hours[hours.length - 1].t)},54 L${xT(hours[0].t)},54 Z`} fill="#00E5FF" fillOpacity={0.18} />
                <path d={path(hours.map((p) => [xT(p.t), 54 - (p.faceFt / maxFace) * 50]))} fill="none" stroke="#00E5FF" strokeWidth={1.8} strokeLinejoin="round" />
                {hours.map((p) => <circle key={p.iso} cx={xT(p.t)} cy={54 - (p.faceFt / maxFace) * 50} r={p.t === activeTime ? 3.5 : 0} fill="#fff" />)}
                <text x={W - 2} y={10} fontSize={10} fill="#9fb1bc" textAnchor="end">{maxFace.toFixed(0)} ft</text>
                {cursor(54)}
              </Row>
              <Row title="Swell period" value={cur?.tp != null ? `${cur.tp.toFixed(0)} s${cur.dp != null ? ` · ${compass(cur.dp)}` : ""}` : "—"} h={40}>
                <path d={path(hours.filter((p) => p.tp != null).map((p) => [xT(p.t), 40 - ((p.tp ?? 0) / maxTp) * 36]))} fill="none" stroke="#8A2BE2" strokeWidth={1.8} strokeLinejoin="round" />
                {hours.filter((p) => p.tp != null && p.hour % 4 === 2 && p.dp != null).map((p) => <g key={p.iso} transform={`translate(${xT(p.t)},${40 - ((p.tp ?? 0) / maxTp) * 36}) rotate(${(p.dp ?? 0) + 180})`}><path d="M0,-5 L3,2 L0,0.5 L-3,2 Z" fill="#c9a2ff" /></g>)}
                <text x={W - 2} y={10} fontSize={10} fill="#9fb1bc" textAnchor="end">{maxTp.toFixed(0)} s</text>
                {cursor(40)}
              </Row>
              <Row title="Wind" value={cur?.windKts != null ? `${Math.round(cur.windKts)} kt${cur.windFrom != null ? ` from ${compass(cur.windFrom)}` : ""}` : "—"} h={44}>
                <path d={path(hours.filter((p) => p.windKts != null).map((p) => [xT(p.t), 30 - ((p.windKts ?? 0) / maxWind) * 26]))} fill="none" stroke="#64d5cc" strokeWidth={1.6} strokeLinejoin="round" />
                {hours.filter((p) => p.windFrom != null && p.hour % 2 === 0).map((p) => <g key={p.iso} transform={`translate(${xT(p.t)},39) rotate(${(p.windFrom ?? 0) + 180})`}><path d="M0,-4.5 L3,2 L0,0.5 L-3,2 Z" fill={p.t === activeTime ? "#fff" : "#64d5cc"} /></g>)}
                <text x={W - 2} y={10} fontSize={10} fill="#9fb1bc" textAnchor="end">{maxWind.toFixed(0)} kt</text>
                {cursor(44)}
              </Row>
              <Row title="Tide" value={tide.hilo.length ? tide.hilo.map((x) => `${x.type}${x.v.toFixed(1)}`).join(" ") + " m" : "—"} h={44}>
                {tide.series.length > 1 && <>
                  ...
                  {tide.hilo.map((x) => <text key={x.t} x={xT(x.t)} y={x.type === "H" ? 8 : 43} fontSize={10} fill="#cfe0ea" textAnchor="middle">{x.type === "H" ? "▲" : "▼"} {x.v.toFixed(1)}</text>)}
                </>}
                {cursor(44)}
              </Row>
              <svg viewBox={`0 0 ${W} 12`} width="100%" height={12} style={{ display: "block" }}>{hourTicks.map((h) => <text key={h} x={xAt(h)} y={10} fontSize={10} fill="#9fb1bc" textAnchor="middle">{tick(h)}</text>)}</svg>
            </>}
          </div>
        ) : (
          <div className="nesurf-drawer-body" style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            {days.map((d, i) => (
              <button key={d.key} onClick={() => onPickDay(i)} style={{ display: "grid", gridTemplateColumns: "58px 18px 1fr auto", alignItems: "center", gap: 8, minHeight: 44, padding: "0 8px", borderRadius: 10, border: 0, cursor: "pointer", textAlign: "left", background: d.active ? "rgba(71,152,183,.22)" : "rgba(255,255,255,.04)", color: "#e8eef2" }}>
                ...
                <span style={{ fontSize: 12, color: "#b8c7d1", whiteSpace: "nowrap" }}>{d.wind}</span>
              </button>))}
          </div>
        )}
      </section>
