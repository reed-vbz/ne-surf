...
| L1 wave energy | **Groundswell: the Rolling Wavefronts shader** (`components/map/RollingWavefrontsLayer.ts`, a BitmapLayer subclass with a custom fragment shader) over the backend wave field: `workers/nesurf/wavefield.py` solves the eikonal equation \|∇t\| = 1/c(h, T) by second-order fast marching on a Gaussian-smoothed CRM grid (180 m) from the deep-water WW3 boundary, with c from the dispersion relation, then H_s by linear shoaling, bottom friction and γ-breaking; one RGBA texture per WW3 step (R,G = travel time 16-bit, B = H_s) plus a static geometry texture (signed distance to shore, depth) published with the cache. The shader reconstructs both with a water-weighted cubic B-spline (16 texels; hardware LINEAR would corrupt the 16-bit packing), draws crests as level sets of φ = ω·t(x) − ω·u_time with the profile I = exp(−(sin φ / w)²), w = 0.08 narrowed in the shallows by c = √(g·max(h, 0.5)) and floored at one pixel, white core → cyan edges, additive blending, intensity tapered to zero over the last 50 m of water, land discarded, and it runs under the land layer. `workers/pinn` is the SWAN-PINN surrogate (U-Net/ConvLSTM, physics loss, FastAPI) that can replace the solver via `NEXT_PUBLIC_PINN_API`. **Wind waves**: the WW3 wind-sea partition as faint fast white particle dashes. | deck.gl custom layer + `TripsLayer` |
...
control became the Forecast control; v3: the search bar became the anticipatory pill band; v4: glass header + full-forecast strip, layer info card removed; v5: the bottom legends/hotspot became the widget dock — uniform 148×164 glass cards on an 8-pt grid, scroll-snap on phones, one row on desktop; v6: the header collapsed to one compact glass bar — a 28 px title row and the 7-day forecast timeline, each day column carrying the region's best range, period and wind with a tier-coloured segment under the slider knob). `node scripts/render-reference.mjs` re-renders `ui-reference.png` from it;
