"use client";
/**
 * Polar swell radar (bottom-left). A 360° compass; every swell partition at the active hour is plotted at its
...
interface Props { trains: SwellTrain[]; facingDeg: number; window: { from_deg: number; to_deg: number }; title: string; sub: string; size?: number }
const R = 78, C = 100;
...
export default function PolarRadar({ trains, facingDeg, window: win, title, sub, size = 156 }: Props) {
  ...
  return (
    <div aria-label="Swell radar" className="backdrop-blur-xl bg-slate-900/60 border border-white/10 rounded-2xl"
      style={{ width: size, padding: 8, boxShadow: "0 12px 32px rgba(0,0,0,.45), 0 0 0 1px rgba(0,229,255,.05), 0 0 28px rgba(0,229,255,.08)", fontFamily: "'Barlow', system-ui, sans-serif", color: "#e8eef2" }}>
      <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase" }}>{title}</div>
      <div style={{ fontSize: 8, color: "#9fb1bc", letterSpacing: ".06em", textTransform: "uppercase", marginBottom: 2 }}>{sub}</div>
