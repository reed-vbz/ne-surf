...
| L4 annotations | `data/spots.json` (the 14 regional breaks), NDBC buoys | deck.gl pulses + buoy status dots, HTML pins + callouts, React hover tooltips, and the glass UI: the **anticipatory pill** (`AnticipatoryPill.tsx`, top-centre where the search bar was: the best break at the active hour surfaces itself with a wind outlook; tap = fly there + open the drawer), the **polar swell radar** (`PolarRadar.tsx`, bottom-left: 360° compass, each WW3 partition at the active hour as a glowing bloom at its direction, radius by period, size by height, colour by energy; windsea scatters, groundswell clusters; swell window arc + facing notch), and the forecast drawer (`ForecastDrawer.tsx`: **wave avatar** `WaveAvatar.tsx` — side-profile wave to scale against a 1.75 m person, height from the forecast, lip curl from period + wind alignment — then 24-hour sparklines for height / period / wind / tide and a 7-day overview; hovering an hour moves the timeline knob and swaps the map's vector fields to that hour) |
...
control became the Forecast control; v3: the search bar became the anticipatory pill band). `node scripts/render-reference.mjs` re-renders `ui-reference.png` from it;
