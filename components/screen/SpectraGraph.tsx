"use client";
/**
 * Buoy swell spectra graph (bottom-left): wave energy across wave period. Observed NDBC spectral density from the nearest
...
interface Props { buoy: { id: string; obs: NdbcBuoy | null } | null; trains: SwellTrain[]; spotName: string; hourLabel: string; size?: number }
const W = 200, H = 120, PL = 8, PR = 6, PT = 10, PB = 18;
const barColor = (T: number) => (T < 7 ? "#ffffff" : T < 9 ? "#7FF6FF" : T < 12 ? "#00E5FF" : T < 15 ? "#2F8CFF" : "#3B4CFF");

export default function SpectraGraph({ buoy, trains, spotName, hourLabel, size = 156 }: Props) {
  ...
  return (
    <div aria-label="Swell spectra" className="backdrop-blur-xl bg-slate-900/60 border border-white/10 rounded-2xl"
